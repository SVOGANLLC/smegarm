...
function Stat({ n, label }: { n: string; label: string }) {
  return (
    <div>
      ...
      <div className="mt-1 text-xs uppercase tracking-[0.2em] text-background/55">{label}</div>
    </div>
  );
}
    ...
    <section id="story" className="relative overflow-hidden bg-foreground text-background">
      <div className="mx-auto grid max-w-[1400px] grid-cols-1 gap-12 px-6 py-28 md:grid-cols-2 md:gap-20 md:px-10 md:py-40">
        ...
        <div className="flex flex-col justify-center">
          <Reveal>
            <p className="eyebrow text-background/60">{t("section.story.eyebrow")}</p>
            <h2 className="mt-4 display-xl text-[clamp(2rem,5vw,4.5rem)] whitespace-pre-line">
