  ...
  return (
    <section className="bg-secondary/40 py-28 md:py-40">
      <div className="mx-auto max-w-[1400px] px-6 md:px-10">
        <SectionHeader eyebrow={t("section.benefits.eyebrow")} title={t("section.benefits.title")} />
          ...
          {items.map((it, i) => (
            <Reveal key={it.n} delay={i * 0.06} className="bg-background">
              <div className="flex h-full flex-col gap-4 p-8 md:p-10">
                ...
                <h3 className="font-serif text-2xl text-foreground">{t(it.tKey)}</h3>
                <p className="text-sm text-muted-foreground">{t(it.dKey)}</p>
              </div>
            </Reveal>
          ))}
        </div>
